...
function getGeneratedSceneName(swfUrl: string): string | null {
  const pathname = (() => {
    try {
  ...
  return fileName?.replace(/\.swf$/i, '') || null;
}
  ...
  const request = (async () => {
    try {
      for (const candidate of [
        `${basePath}/DefineSprite_${id}/1.svg`,
        `${basePath}/DefineSprite_${id}/1.png`,
        url,
        `${basePath}/${id}.svg`,
      ]) {
